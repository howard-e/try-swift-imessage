...
class MessagesViewController: MSMessagesAppViewController {
    ...
    private func composeMessage(with event: Event) -> MSMessage {
		...
		guard let title = event.title, let date = event.date, let image = event.image else {
			return message
		}
		...
		if let image = event.image { layout.image = UIImage(named: image) }
		if let title = event.title { layout.caption = title }
		if let date = event.date { layout.trailingCaption = date }
		layout.subcaption = "😊: \(event.attending) | ☹️: \(event.notAttending)"
...
extension MessagesViewController: CreateViewControllerDelegate {
    ...
    func didCreate(with event: Event) {
        requestPresentationStyle(.compact) // when user sends event, minimize
